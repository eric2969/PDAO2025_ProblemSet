...
whether you've already win, i.e. you can't be \textbf{checkmated} while black has \textbf{no available moves} to prevent themself from being \textbf{checked} or \textbf{stalemate}.
\\\\
\noindent
...
\\\noindent
\textbf{\includegraphics{Xiangqi.assets/75px-Xiangqi_General.png}General(G):}
...
\\\noindent
\textbf{\includegraphics{Xiangqi.assets/75px-Xiangqi_Advisor.png}Advisor(S):}
...
\\\noindent
\textbf{\includegraphics{Xiangqi.assets/75px-Xiangqi_Elephant.png}Elephant(E):}
...
\\\noindent
\textbf{\includegraphics{Xiangqi.assets/75px-Xiangqi_Chariot.png}Chariot(R):}
...
\\\noindent
\textbf{\includegraphics{Xiangqi.assets/75px-Xiangqi_Cannon.png}Cannon(C):}
...
over to its target.
\\\noindent
\textbf{\includegraphics{Xiangqi.assets/75px-Xiangqi_Horse.png}Horse(H):}
...
leg}.
\\\noindent
\textbf{\includegraphics{Xiangqi.assets/75px-Xiangqi_Soldier.png}Soldier(S):}
...
\\\\
\noindent
You're given a later game board. Now it's black's turn to move, determine whether it's possible for you, i.e. the red/bottom player to capture the black general immediately after the black player's turn. However, if you're already checked, the black player will, without a doubt, capture your general, which indicates your lost. Please be noted that in Xiangqi. Each player is forced to make one move in their turn. Therefore, being stalemated also count as losing.
\\
\subsection*{\fontsize{16}{12}Input}
...
Next \textbf{\( N \)} lines, each contains a command:
    \begin{itemize}
        ...
        \item \textbf{D P} $\rightarrow$ Delete a class at position \( P \) \((1 \leq P \leq 1000)\). It's guaranteed that the deletion only happens when the position given exists a class.
    \end{itemize}
