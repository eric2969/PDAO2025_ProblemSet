...
\begin{document}
\title{\textbf{Programming Design And Optimization}\\ April 19, 2025}
\date{}
\maketitle
\thispagestyle{fancy}
...
\end{itemize}
%\includegraphics[width=0.3\textwidth]{Line.JPG}
\begin{center}
...
\begin{center}
    \begin{tikzpicture}[scale=0.9]
        ...
        \begin{scope}[shift={(-6.25,0)}] % Adjust position for first diagram
            ...
            \foreach \i/\label in {90/1, -54/3, -126/4, 162/5} {
                \node[circle, draw, fill=white!50, minimum size=1cm] (\label) at (\i:2.5cm) {\textbf{\label}};
            }
            % Draw eliminated pirate
            \node[circle, draw, fill=white!50, minimum size=1cm] (2) at (18:2.5cm) {\textbf{2}};

            % Label step number
            \node at (-3, 2.5) {\textbf{1)}};
        \end{scope}

        % Second Circle (After eliminating 2)
        ...
        \begin{scope}[shift={(0,0)}] % Adjust position for second diagram
            ...
            \foreach \i/\label in {90/1, -54/3, 162/5} {
                \node[circle, draw, fill=white!50, minimum size=1cm] (\label) at (\i:2.5cm) {\textbf{\label}};
            }
            % Draw eliminated pirate
            \node[circle, draw, fill=white!50, minimum size=1cm] (4) at (-126:2.5cm) {\textbf{4}};
            
            % Label step number
            \node at (-3, 2.5) {\textbf{2)}};
        \end{scope}

        % Third Circle (After eliminating 4)
        ...
        \begin{scope}[shift={(6,0)}] % Adjust position for third diagram
            ...
            % Draw remaining pirates
            \foreach \i/\label in {-54/3, 162/5} {
                \node[circle, draw, fill=white!50, minimum size=1cm] (\label) at (\i:2.5cm) {\textbf{\label}};
            }
            % Draw eliminated pirate
            \node[circle, draw, fill=white!50, minimum size=1cm] (1) at (90:2.5cm) {\textbf{1}};
    ...
    \begin{tikzpicture}[scale=0.9] % Adjust scale to fit better
        ...
        % Fourth Circle (After eliminating 1)
        \begin{scope}[shift={(-3,-4)}] 
            % Draw eliminated pirate (red)
            \node[circle, draw, fill=white!50, minimum size=1.2cm] (5) at (162:2.5cm) {\textbf{5}};
        \end{scope}
        ...
        \begin{scope}[shift={(-3,-4)}] % Position fourth diagram
            ...
            \node[circle, draw, fill=white!50, minimum size=1cm] (3) at (-54:2.5cm) {\textbf{3}};
            
            % Draw eliminated pirate (red)
            \node[circle, draw, fill=white!50, minimum size=1cm] (5) at (162:2.5cm) {\textbf{5}};
...
\begin{itemize}
    ...
    \item Every night, during the Moonlight Ritual, you may declare the shape of your shadow. If you are correct, you will be awaken; if you are wrong, you will be forever lost in darkness.
    \item The chosen individuals cannot communicate with each other and must deduce their shadow's shape over time.  
    \item Everyone in this trial plays optimally, meaning they will only declare their shadow shape when they are sure of it.
\end{itemize}
\noindent
The trial's objective is clear:
The first kingdom whose envoys all successfully identify their own shadow shapes and no one lost in darkness will claim ultimate victory.
...
\subsection*{\fontsize{16}{12}Explanation}
Since we know those two countries will all identify their shadows in the same night, but country 1 has more people.
\\\\
\noindent
