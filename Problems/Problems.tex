...
\subsection*{\fontsize{16}{12}Explanation}
\noindent
After the rainfall, there will be \textbf{10} unit of water being trapped, since the water collected in the low areas escaped at the edge.
...
\begin{center}
    \begin{minipage}{0.48\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \node (6) at (0, -4) [circle, draw, minimum size=1cm] {6};
            \node (5) at (-1, -2.5) [circle, draw, minimum size=1cm] {5};
            ...
            \draw[gray, very thick] (3) -- node[right] {1} (4);   % edge 4
            \draw[gray, very thick] (4) -- node[below] {1} (6);   % edge 5
            \draw[gray, very thick] (6) -- node[left] {1} (5);    % edge 6

            % Backup-Ready Edges
            \draw[dashed, thick, gray] (5) -- node[above] {1} (3);  
        \end{tikzpicture}}
        \captionof*{figure}{MST with critical edges: 1, 2, 3}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \scalebox{1.1}{
        \begin{tikzpicture}
            ...
            % Alternate MST using backup-ready edge
            \draw[gray, very thick] (3) -- node[right] {1} (4);   % edge 4
            \draw[gray, very thick] (4) -- node[below] {1} (6);   % edge 5
            \draw[gray, very thick] (6) -- node[left] {1} (5);    % edge 6
            \draw[gray, very thick] (5) -- node[below] {1} (3);   % edge 7
        \end{tikzpicture}}
        \captionof*{figure}{Backup-ready cycle: edges 4, 5, 6, 7}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \scalebox{1.1}{
        ...
        \end{tikzpicture}}
        \captionof*{figure}{If channel 1 is removed, the cheapest way to reconnect node 1 is via channel 0 with cost 6, increasing total MST cost from 7 to 12.}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \scalebox{1.1}{
...
\begin{itemize}
    ...
    \item The mathematical representation of the extraction process is as follows:\begin{itemize}
        \item $a_i = c_{i\%N} \oplus c_{(i+1)\%N} \oplus \cdots \oplus c_{(i+k_1-1)\%N},\ i\in [0,N)$ \\
        \item $b_i = c_{i\%N} \oplus c_{(i+1)\%N} \oplus \cdots \oplus c_{(i+k_2-1)\%N},\ i\in [0,N)$
        \end{itemize}
\end{itemize}
\noindent
...
\\\\
\noindent
Line 2 and 3 each contains $N$ integers $a_i$ and $b_i$, the message to be extracted by type-$A$ and type-$B$ drones, respectively.
...
\subsection*{\fontsize{16}{12}Output}
Print $N$ non-negative integers $c_i,i\in [0,N)$, denoting the encrypted signal such that the XOR constraints allow recovery of both messages.\\\\\noindent
It is promoised that there \textbf{exists at least one valid} $c$ extracted to recover both messages.
\\\\
\noindent
If there are multiple valid arrays, print any \textbf{one of them}. any number you write to the output must be \textbf{in the range} of $[0, 2^{30})$, or you'll get a "\textbf{Wrong Answer}".
\subsection*{\fontsize{16}{12}Examples}
\begin{table}[h]
    \centering
    \begin{tabularx}{\textwidth}{|>{\ttfamily}X|>{\ttfamily}X|}
