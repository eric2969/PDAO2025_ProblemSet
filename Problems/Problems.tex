...
leg}.
\\\noindent
\textbf{\includegraphics{Xiangqi.assets/75px-Xiangqi_Soldier.png}Soldier(S,s):}
...
\\
\subsection*{\fontsize{16}{12}Input}
Given a $10\times9$ array full of characters, where the general, advisor, elephant, chariot, horse, canon, soldier, and empty space are represented by G, S, E, R, H, C, P, and . (a single dot) respectively. Upper cases indicate \textbf{Black player} while lower cases indicate \textbf{Red player}.
\subsection*{\fontsize{16}{12}Output}
...
each group \(i \in [0,n)\) will consist of \(M_i\) pirates, and the group will be assigned an elimination number \(K_i\).
...
\begin{itemize}
    \item If a sensor originally had a higher power level than another sensor in the \textbf{same row or column}, it must remain more powerful in the new configuration. i.e. Let \( P' \) be the reconfigured security system of the original one, say, \( P \). Then, for all indices \( i, j_1, j_2 \) such that \( j_1 > j_2 \), the following equivalence holds:
    \[
    P'_{i, j_1} > P'_{i, j_2} \iff P_{i, j_1} > P_{i, j_2}.
    \]
    
    Similarly, for all indices \( j, i_1, i_2 \) such that \( i_1 > i_2 \), we have:
    \[
...
\begin{itemize}
    ...
    \item $P_{i_1, j_1} \neq P_{i_2, j_2}, \quad \forall (i_1, j_1) \neq (i_2, j_2), i_1, i_2 \in [0, M-1], j_1, j_2 \in [0, N-1]$
...
\subsection*{\fontsize{16}{12}Output}
Print "Yes" if the two formulas have same number of atoms; otherwise, print "No".
...
\begin{itemize}
    \item \textbf{Channels 0 and 1} are critical because removing either would increase the cost.
    \item \textbf{Channels 2, 3, 4, and 5} are backup-ready since they can replace another backup-ready channel without increasing the cost.
\end{itemize}
\begin{center}
    \begin{minipage}{0.48\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \draw[very thick] (0) -- node[above] {1} (1);
            \draw[very thick] (1) -- node[above right] {1} (2);
            \draw[gray, very thick] (2) -- node[below] {2} (3);
            \draw[gray, very thick] (0) -- node[left] {3} (4);
        \end{tikzpicture}}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \draw[very thick] (0) -- node[above] {1} (1);
            \draw[very thick] (1) -- node[above right] {1} (2);
            \draw[gray, very thick] (0) -- node[below left] {2} (3);
            \draw[gray, very thick] (3) -- node[below left] {3} (4);
        \end{tikzpicture}}
    \end{minipage}
\end{center}
...
\subsection*{\fontsize{16}{12}Explanation}
All channels have equal cost, so there are multiple distinct minimum-cost networks possible. Any channel can be replaced by another without increasing the cost.

\begin{itemize}
    \item There are \textbf{no critical channels} since removing any one of them still allows a different minimum-cost spanning tree of the same total cost.
    \item \textbf{All channels} are backup-ready, as they can seamlessly replace others in the network without increasing cost.
\end{itemize}
...
\begin{center}
    \begin{minipage}{0.48\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \node (0) at (0, 0) [circle, draw, minimum size=1cm] {0};
            \node (1) at (3, 0) [circle, draw, minimum size=1cm] {1};
            \node (2) at (3, -3) [circle, draw, minimum size=1cm] {2};
            \node (3) at (0, -3) [circle, draw, minimum size=1cm] {3};
            
            % Draw MST 1
            \draw[gray, very thick] (0) -- node[above] {1} (1);
            \draw[gray, very thick] (1) -- node[right] {1} (2);
            \draw[gray, very thick] (2) -- node[below] {1} (3);
        \end{tikzpicture}}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        ...
        \begin{tikzpicture}
            ...
            % Draw MST 2
            \draw[gray, very thick] (0) -- node[left] {1} (3);
            \draw[gray, very thick] (3) -- node[below] {1} (2);
            \draw[gray, very thick] (2) -- node[right] {1} (1);
        \end{tikzpicture}}
    \end{minipage}
\end{center}
...
Next \textbf{\( N \)} lines, each contains a command:
    \begin{itemize}
        ...
        \item \textbf{D P} $\rightarrow$ Delete a class at position \( P \) \((1 \leq P \leq 1000)\). Position $P$ must contain a class.
    \end{itemize}
